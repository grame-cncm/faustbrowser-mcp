...
from faust_doc_tools import (
    FaustDocStore,
    explain_faust_symbol_for_goal as doc_explain_faust_symbol_for_goal,
    get_faust_examples as doc_get_faust_examples,
    get_faust_symbol as doc_get_faust_symbol,
    list_faust_module as doc_list_faust_module,
    search_faust_lib as doc_search_faust_lib,
)
...
def _get_faust_doc_store() -> FaustDocStore:
    """Load and cache the configured split Faust doc store in-process."""

    global FAUST_DOC_STORE
    if FAUST_DOC_INDEX_CANDIDATE is None:
        raise RuntimeError(
            "No Faust doc index configured. Set FAUST_DOC_INDEX or pass "
            "--faust-doc-index with a split index generated by faustlibraries."
        )
    if FAUST_DOC_STORE is None:
        FAUST_DOC_STORE = FaustDocStore.from_candidate(FAUST_DOC_INDEX_CANDIDATE)
    return FAUST_DOC_STORE
...
@mcp.tool()
def search_faust_lib(query: str, limit: int = 10, module: str | None = None) -> str:
    """Search the split Faust doc index without loading full module docs into context."""

    result = doc_search_faust_lib(_get_faust_doc_store(), query, limit=limit, module=module)
    return json.dumps(result, indent=2)


@mcp.tool()
def get_faust_symbol(symbol: str) -> str:
    """Return the full documentation entry for one Faust library symbol."""

    result = doc_get_faust_symbol(_get_faust_doc_store(), symbol)
    return json.dumps(result, indent=2)


@mcp.tool()
def list_faust_module(module: str, limit: int = 200) -> str:
    """List lightweight symbol summaries from one Faust library module."""

    result = doc_list_faust_module(_get_faust_doc_store(), module, limit=limit)
    return json.dumps(result, indent=2)


@mcp.tool()
def get_faust_examples(symbol_or_module: str, limit: int = 10) -> str:
    """Return test/example snippets for a Faust symbol or module."""

    result = doc_get_faust_examples(_get_faust_doc_store(), symbol_or_module, limit=limit)
    return json.dumps(result, indent=2)


@mcp.tool()
def explain_faust_symbol_for_goal(symbol: str, goal: str) -> str:
    """Build a short action-oriented recommendation from one Faust doc entry."""

    result = doc_explain_faust_symbol_for_goal(_get_faust_doc_store(), symbol, goal)
    return json.dumps(result, indent=2)
...
def main() -> None:
    ...
    parser.add_argument(
        "--faust-doc-index",
        default=FAUST_DOC_INDEX,
        help=(
            "Path to a split Faust doc index file or directory generated by "
            "faustlibraries make doc-index-split."
        ),
    )
    args = parser.parse_args()

    global FAUST_DOC_INDEX_CANDIDATE
    global FAUST_DOC_STORE
    FAUST_DOC_INDEX_CANDIDATE = Path(args.faust_doc_index).resolve() if args.faust_doc_index else None
    FAUST_DOC_STORE = None
