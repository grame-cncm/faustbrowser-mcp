...
def _new_faust_doc_store() -> FaustDocStore:
    """Build a fresh Faust doc store from the configured split index path."""
    ...
    return FaustDocStore.from_candidate(FAUST_DOC_INDEX_CANDIDATE)
...
def search_faust_lib(query: str, limit: int = 10, module: str | None = None) -> str:
    ...
    result = doc_search_faust_lib(_new_faust_doc_store(), query, limit=limit, module=module)
    return json.dumps(result, indent=2)
...
def get_faust_symbol(symbol: str) -> str:
    ...
    result = doc_get_faust_symbol(_new_faust_doc_store(), symbol)
    return json.dumps(result, indent=2)
...
def list_faust_module(module: str, limit: int = 200) -> str:
    ...
    result = doc_list_faust_module(_new_faust_doc_store(), module, limit=limit)
    return json.dumps(result, indent=2)
...
def get_faust_examples(symbol_or_module: str, limit: int = 10) -> str:
    ...
    result = doc_get_faust_examples(_new_faust_doc_store(), symbol_or_module, limit=limit)
    return json.dumps(result, indent=2)
...
def explain_faust_symbol_for_goal(symbol: str, goal: str) -> str:
    ...
    result = doc_explain_faust_symbol_for_goal(_new_faust_doc_store(), symbol, goal)
    return json.dumps(result, indent=2)
